...
void muestraUnNodoPedido(nodoPedido* pedido)
{
    ...
    muestraUnaCabezeraPedido(pedido->datoPedido);
    printf("ID COSTO PEDIDO......: %.2f\n", pedido->costoPedido);
}
...
nodoPedido* buscarNodoIdPedido(nodoPedido* lista, int id)
{
    ...
    while((aux != NULL) && (aux->datoPedido.idCabezeraPedido != id))///recorro la lista mientras no finalize o no se encuentre el nodo
    {
        aux = aux->siguiente;
    }
...
nodoPedido* borrarNodoIdPedido(nodoPedido* lista, int id)
{
    ...
    if(lista != NULL && lista->datoPedido.idCabezeraPedido == id)///si la lista no esta vacia y el primer nodoPedido es el buscado se elimina
    {
    ...
    else
    {
        ...
        while((seg != NULL) && seg->datoPedido.idCabezeraPedido != id)///recorre la lista mientras no sea NULL y no se encuentre el dato
        {
            ante = seg;
            seg = seg->siguiente;
        }
...
void listaPedido2CabeceraPedido(nodoPedido* listaPedido, char nombreArch[])///carga archivo de cabecera
{
    ...
    if(arch)
    {
        while(aux)
        {
            c.idCabezeraPedido += id;
            strcpy(c.fechaPedido, aux->datoPedido.fechaPedido);

            c.idCabezeraPedido = aux->datoPedido.idCabezeraPedido;
