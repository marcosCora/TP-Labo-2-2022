...
///funcion que pasa los datos de la cabecera de pedido y los vincula con cada cliente
/**
nodoPedido* archivoCabeceraPedido2ListaPedido(nodoPedido* pedido, FILE* archivoCabecera, int idCliente)
{
    ...
    if(arch)
    {
        while(fread(&c, sizeof(stCabeceraPedido), 1, arch) > 0)
        {
            ...
            {
                pedido = agregarPrincipioListaCliente(pedido, creaNodoPedido(c));
